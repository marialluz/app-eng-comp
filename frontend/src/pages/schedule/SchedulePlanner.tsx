...
const SchedulePlanner: React.FC = () => {
        ...
        setSemesters((prevSemesters) => {
          ...
          if (existingSemester) {
            return prevSemesters.map((s) =>
              s.period === currentPeriod ? { ...s, subjects: [...s.subjects, subject] } : s,
            )
          } else {
  ...
  const handleSavePlan = async () => {
    try {
      ...
      // Assuming the API returns the plan ID in the response
      const response = await api.post("/schedule/", planData)
      const planId = (response.data as { id: string }).id

      // If we have a plan ID, update it
      if (planId) {
        await api.put(`/schedule/${planId}/`, planData)
      }
          ...
          <Grid item xs={12} md={4}>
            <Paper elevation={3} sx={{ p: 2 }}>
              ...
              <TextField
                ...
                sx={{ mb: 2 }}
              />
              <Button variant="contained" color="primary" startIcon={<AddIcon />} onClick={handleAddSubject} fullWidth>
                Adicionar Disciplina
              </Button>
            </Paper>
          </Grid>
                  ...
                  <List>
                    {semester.subjects.map((subject) => (
                      <ListItem key={subject.code}>
                        <ListItemText primary={subject.name} secondary={`Código: ${subject.code}`} />
                        <ListItemSecondaryAction>
                          <IconButton
                            edge="end"
                            aria-label="delete"
